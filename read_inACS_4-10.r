...
SumEmpData <- FilterEmpData %>% dplyr::group_by( area_fips, industry_code) %>% dplyr::summarize(Emp2013 = sum(annual_avg_emplvl) )
...
data_acs_age <- data_acs_emp1J %>% 
  dplyr::group_by(IND1, czone, HHWT) %>%   ###################### XXXXXXXXXXXX
  # dplyr::tally  %>% 
...
data_acs_age1hh <- data_acs_age %>% dplyr::group_by(IND1, czone, ageg, HH) %>%
          dplyr::summarise( AGE1 = sum( HHWT), avghhworkers = mean(workers) )
          # %>% arrange(czone, HH, ageg, IND1 )
